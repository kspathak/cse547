...
# Data flags
flags.DEFINE_list('training_data', ['data/patch_features_tiny/train2014.p'],
                  'Data to train model.')
...
flags.DEFINE_string('train_s3_log_key',
                    'project/train/training_log_{0}.pkl'.format(os.getenv('AWS_BATCH_JOB_ID', time.strftime('%s'))),
                    'Key in AWS S3 bucket to dump ouput to.')
flags.DEFINE_string('train_s3_model_key',
                    'project/train/model_{0}.pkl'.format(os.getenv('AWS_BATCH_JOB_ID', time.strftime('%s'))),
...
def main(argv):
    dataset = CocoPatchesDataset.from_state_dict_files(FLAGS.training_data)
